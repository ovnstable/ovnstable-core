...
contract Mark2Market is IMark2Market, OwnableExt {
    ...
    function activePrices(address _addActive) public view override returns (ActivesPrices memory) {
        IActivesList.Active memory active = actListContr.getActive(_addActive);

        IERC20Metadata tokAct = IERC20Metadata(active.actAddress);
        uint price = IConnector(active.connector).getPriceOffer(active.actAddress,
            active.poolPrice);
        uint bookValue = IConnector(active.connector).getBookValue(active.actAddress,
            addrWault, active.poolPrice);
        uint liqValue = IConnector(active.connector).getLiqValue(active.actAddress,
            addrWault, active.poolPrice);

        return ActivesPrices(active.actAddress,
            tokAct.name(),
            tokAct.symbol(),
            tokAct.decimals(),
            price,
