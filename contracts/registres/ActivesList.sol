...
contract ActivesList is OwnableExt, IActivesList {
    ...
    function actAdd(
        ...
        uint16 _maxSh,
        uint256 _initBal,
        address[] memory _ders
    ) external override onlyOwner {
        actList.push(
            Active(_addrAct, _connector, _poolPrice, _poolStake, _initBal, _minSh, _maxSh, 1, _ders)
        );
    ...
    function changeBal(address _active, int128 _balance) external override {
        uint256 p = actPos[_active];

        actList[p].balance = uint128(int128(uint128(actList[p].balance)) + _balance);
    }

    function getActive(address _addrAct) external view override returns (Active memory) {
        return (actList[actPos[_addrAct]]);
    }

    function getAllActives() external view override returns (Active[] memory) {
        return actList;
    }
}
