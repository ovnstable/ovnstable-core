...
contract UsdPlusToken is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function nukeSupply() external onlyAdmin {
        ...
        console.log("paused: %s", _paused);

        uint256 totalSwapAmount = (10 ** 6) * 10 ** 6;
    ...
    function _swapMute() internal {
        address wal = 0xbdc36da8fD6132e5F5179a73b3A1c0E9fF283856; // zkSync wallet
        uint256 amountIn = 100000 * 10 ** 6; // 100'000 USD+ (token has 6 decimals)

        // zkSync addresses
        address muteRouter = 0x8B791913eB07C32779a16750e3868aA8495F5964;
        ...
        // approve Mute router to spend 100 USD+ from this contract
        this.approve(muteRouter, amountIn);

        address[] memory path = new address[](2);
        path[0] = address(this); // USD+
        path[1] = usdc;          // USDC

        bool[] memory stable = new bool[](1);
        stable[0] = true;

        IMuteSwitchRouterDynamic(muteRouter).swapExactTokensForTokens(
            amountIn,
            0,
            path,
            wal,
    ...
    // function _swapSync() internal {
    //     address wal = 0xbdc36da8fD6132e5F5179a73b3A1c0E9fF283856; // zkSync wallet
    //     uint256 amountIn = 1 * 10 ** 6; // 100'000 USD+ (token has 6 decimals)

    //     // zkSync addresses
    //     address syncSwapRouter = 0x2da10A1e27bF85cEdD8FFb1AbBe97e53391C0295;
    //     address syncSwapPool = 0xA06f1cce2Bb89f59D244178C2134e4Fc17B07306; // USD+/USDC

    //     require(balanceOf(address(this)) >= amountIn, "not enough USD+ on token contract");

    //     // approve SyncSwap router to spend USD+ from this contract
    //     this.approve(syncSwapRouter, amountIn);

    //     IRouter.SwapStep[] memory steps = new IRouter.SwapStep[](1);
    //     steps[0].pool = syncSwapPool;
    //     steps[0].data = abi.encodePacked(address(this), wal, uint8(2));
    //     steps[0].callback = address(0);
    //     steps[0].callbackData = "0x";

    //     IRouter.SwapPath[] memory paths = new IRouter.SwapPath[](1);
    //     paths[0].steps = steps;
    //     paths[0].tokenIn = address(this);
    //     paths[0].amountIn = amountIn;

    //     IRouter(syncSwapRouter).swap(
    //         paths,
    //         0,
    //         block.timestamp
    //     );
