...
function loadProxyAddress() {
  ...
  const p = path.join(__dirname, "../deployments/StrategyZerolend.json");
  if (fs.existsSync(p)) {
  ...
  throw new Error(
    "Proxy address not provided. Set PROXY_ADDRESS or add address to deployments/StrategyZerolend.json",
  );
}
...
module.exports = async function (hre) {
  ...
  const artifact = await deployer.loadArtifact("StrategyZerolend");
  const proxyAddress = loadProxyAddress();
  console.log(`[proxy] ${proxyAddress}`);

  console.log(`[deploy] deploying StrategyZerolend impl (onlyImpl=${ONLY_IMPL})...`);
