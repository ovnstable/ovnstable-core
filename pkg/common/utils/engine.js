...
async function engineSendTxSigned(unsignedTransaction, walletAddress) {
    ...
    try {
        ...
        console.log(`tx submitted. queueId: ${queueId}`);
        ...
        while (true) {
            ...
            if (status.status === "mined") {
                console.log(`tx ${queueId} successfully mined!`);
                return;
            }

            if (status.status === "errored") {
                console.error(`tx ${queueId}: ${status.errorMessage}`);
