...
task('deploy', 'deploy')
    ...
    .setAction(async (args, hre) => {
        ...
        if (args.reset) await evmCheckpoint('task', hre.network.provider);
        ...
        if (args.reset) await evmRestore('task', hre.network.provider);
    });
...
task(TASK_NODE, 'Starts a JSON-RPC server on top of Hardhat EVM')
    ...
    .setAction(async (args, hre, runSuper) => {
        ...
        if (args.last) {
            ...
            process.env.HARDHAT_BLOCK_NUMBER = String(block);
            process.env['HARDHAT_BLOCK_NUMBER_' + process.env.ETH_NETWORK.toUpperCase()] = String(block);
        }

        if (args.deploy) args.noDeploy = false;
        else args.noDeploy = true;

        if (args.reset) args.noReset = false;
        else args.noReset = true;
...
task(TASK_COMPILE, 'Compile').setAction(async (args, hre, runSuper) => {
    args.quiet = true;
