  ...
  tables.push(
    await testCase(async () => {
      await execTimelock(async (timelock) => {
        ...
        await (
          await roleManager.connect(timelock).grantRole(Roles.UNIT_ROLE, timelock.address, await getPrice())
        ).wait()
        await (
          await exchange.connect(timelock).setPayoutTimes(1637193600, 24 * 60 * 60, 15 * 60, await getPrice())
        ).wait()
...
async function testStrategy(id, strategy, stand = process.env.STAND) {
  ...
  tables.push(
    await testCase(async () => {
      await execTimelock(async (timelock) => {
        let amount = toAsset(10_000, stand)
