...
async function main() {
    ...
    let timelockNew =  await getContract('AgentTimelock', 'base');
    let timelockOld =  await getContract('OvnTimelockController', 'base');
    ...
    for (const name of coreNames) {
        let contractUsdPlus = await getContract(name, 'base');
        contracts.push(contractUsdPlus.address);

        try {
            let contractDaiPlus = await getContract(name, 'base_dai');
