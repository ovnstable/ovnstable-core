...
async function main() {
    ...
    let exUsdc = await getContract('Exchange', 'base_usdc');
    let ex = await getContract('Exchange', 'base');
    // let rmUsdc = await getContract('RoleManager', 'base_usdc');
    // let rm = await getContract('RoleManager', 'base_usdc');

    const exNew = '0x10a013eB5D9F23DcD4b8F51321D6eA7d462927d4';

    addProposalItem(ex, "upgradeTo", [exNew]);
    addProposalItem(exUsdc, "upgradeTo", [exNew]);
