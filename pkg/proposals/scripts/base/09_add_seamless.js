...
async function main() {
    ...
    let strategy = await getContract('StrategySeamless', 'base');
    let pm = await getContract('PortfolioManager', 'base');

    let strategyDai = await getContract('StrategySeamlessDai', 'base_dai');
    ...
    values.push(0);
    abis.push(pmDai.interface.encodeFunctionData('addStrategy', [strategyDai.address]));
