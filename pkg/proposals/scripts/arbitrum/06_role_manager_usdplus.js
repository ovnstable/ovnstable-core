...
async function main() {
    ...
    let silo = await getContract('StrategySiloUsdc');

    addresses.push(silo.address);
    values.push(0);
    abis.push(silo.interface.encodeFunctionData('upgradeTo', ['0x12BF678345ff4e647E81FA3C3AdCA00A2863DD6E']));

    addresses.push(silo.address);
    values.push(0);
    abis.push(silo.interface.encodeFunctionData('setStrategyParams', [pm.address, roleManager.address]));
    ...
    await testStrategy(filename, silo);
