...
async function main() {
    ...
    let timelockNew =  await getContract('AgentTimelock', 'polygon');
    let timelockOld =  await getContract('OvnTimelockController', 'polygon');
    ...
    for (const name of coreNames) {
        let contractUsdPlus = await getContract(name, 'polygon');
        contracts.push(contractUsdPlus.address);
    }


    const fileStream = fs.createReadStream('polygon_strategies.txt');
