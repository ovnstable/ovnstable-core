...
contract BeefyVelodromeZap is OdosZap {
    ...
    function zapIn(SwapData memory swapData, BeefyAerodromeZapInParams memory velodromeData) external {
        _prepareSwap(swapData);
        _swap(swapData);

        IBeefyVaultV7 gauge = IBeefyVaultV7(velodromeData.gauge);
