...
describe("ArbidexZapper", function () {
    ...
    sharedBeforeEach('deploy and setup', async () => {
        ...
        await deployments.fixture(['ArbidexZap']);
    ...
    it("swap and put nearly equal", async function () {
        ...
        const outputTokens = proportions.outputTokens.map(({ tokenAddress }) => {
            return { "tokenAddress": tokenAddress, "receiver": arbidexZap.address };
        });


        const receipt = await (await arbidexZap.connect(account).zapIn({
            inputs: inputTokens,
...
async function getOdosRequest(request) {
    ...
    try {
        ...
        console.log("[arbidexZap] getSwapTransaction: " + e);
        return 0;
    }

    if (transaction.statusCode === 400) {
        console.log(`[arbidexZap]  ${transaction.description}`);
        return 0;
    }

    if (transaction.data.transaction === undefined) {
        console.log("[arbidexZap] transaction.tx is undefined");
        return 0;
    }
