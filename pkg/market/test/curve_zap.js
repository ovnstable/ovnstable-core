...
const {
    ...
    initWallet,
    execTimelock,
    getContract, getContractByAddress, getERC20ByAddress
...
describe("CurveZapper", function () {
    ...
    sharedBeforeEach('deploy and setup', async () => {
        ...
        await deployments.fixture(['CurveZap']);

        account = await setUp();
        curveZap = await ethers.getContract("CurveZap");

        token0Out = (await getContract('UsdPlusToken', 'arbitrum')).connect(account);
        token1Out = (await getERC20ByAddress(ARBITRUM.fraxbp)).connect(account);
    ...
    it.only("swap and put nearly equal", async function () {
        ...
        const gauge = "0x4645e6476d3a5595be9efd39426cc10586a8393d";
        ...
        await (await token0In.approve(curveZap.address, amountToken0In)).wait();
        await (await token1In.approve(curveZap.address, amountToken1In)).wait();
        await (await token0Out.approve(curveZap.address, amountToken0Out)).wait();
        await (await token1Out.approve(curveZap.address, amountToken1Out)).wait();

        const reserves = await curveZap.getProportion(gauge);
        const sumReserves = reserves[0].add(reserves[1])
        console.log("reserves", reserves[0].toString(), reserves[1].toString(), sumReserves.toString());
        console.log("proportion", reserves[0] / (sumReserves), reserves[1] / (sumReserves));
        ...
        const outputTokens = proportions.outputTokens.map(({ tokenAddress }) => {
            return { "tokenAddress": tokenAddress, "receiver": curveZap.address };
        });

        console.log("ZapIn. Input tokens: ", inputTokens);
        console.log("ZapIn. Output tokens: ", outputTokens);

        const receipt = await (await curveZap.connect(account).zapIn({
            inputs: inputTokens,
        ...
        expect(fromE6(await token0In.balanceOf(curveZap.address))).to.lessThan(1);
        expect(fromE18(await token1In.balanceOf(curveZap.address))).to.lessThan(1);
        expect(fromE6(await token0Out.balanceOf(curveZap.address))).to.lessThan(1);
        expect(fromE18(await token0Out.balanceOf(curveZap.address))).to.lessThan(1);
...
async function getOdosRequest(request) {
    ...
    try {
        ...
        console.log("[CurveZap] getSwapTransaction: " + e);
        return 0;
    }

    if (transaction.statusCode === 400) {
        console.log(`[CurveZap]  ${transaction.description}`);
        return 0;
    }

    if (transaction.data.transaction === undefined) {
        console.log("[CurveZap] transaction.tx is undefined");
        return 0;
    }
...
async function setUp() {
    ...
    await transferAsset(ARBITRUM.fraxbp, account.address);
