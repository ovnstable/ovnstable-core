...
describe("DemetorZapper", function () {
    ...
    sharedBeforeEach('deploy and setup', async () => {
        ...
        await deployments.fixture(['DemetorZap']);

        account = await setUp();
        demetorZap = await ethers.getContract("DemetorZap");

        token1Out = (await getContract('UsdPlusToken', 'arbitrum')).connect(account);
        token0Out = "0xD74f5255D557944cf7Dd0E45FF521520002D5748";
    ...
    it("swap and put nearly equal", async function () {
        ...
        const amountToken0In = toE18(300);
        const amountToken1In = toE18(300);
        const amountToken1Out = toE6(100);
        const amountToken0Out = toE18(0);

        await (await token0In.approve(demetorZap.address, amountToken0In)).wait();
        await (await token1In.approve(demetorZap.address, amountToken1In)).wait();
        // await (await token0Out.approve(demetorZap.address, amountToken0Out)).wait();
        await (await token1Out.approve(demetorZap.address, amountToken1Out)).wait();

        const reserves = await demetorZap.getProportion(gauge);
        ...
        const proportions = calculateProportionForPool({
            ...
            outputTokensAddresses: ["0xD74f5255D557944cf7Dd0E45FF521520002D5748", token1Out.address],
        ...
        const outputTokens = proportions.outputTokens.map(({ tokenAddress }) => {
            return { "tokenAddress": tokenAddress, "receiver": demetorZap.address };
        });

        const receipt = await (await demetorZap.connect(account).zapIn({
            inputs: inputTokens,
        ...
        expect(token0Out).to.equals(putIntoPoolEvent.args.tokensPut[0]);
        expect(token1Out.address).to.equals(putIntoPoolEvent.args.tokensPut[1]);

        expect(token0Out).to.equals(returnedToUserEvent.args.tokensReturned[0]);
        ...
        expect(fromE6(await token0In.balanceOf(demetorZap.address))).to.lessThan(1);
        expect(fromE18(await token1In.balanceOf(demetorZap.address))).to.lessThan(1);
        expect(fromE6(await token1Out.balanceOf(demetorZap.address))).to.lessThan(1);
        // expect(fromE18(await token0Out.balanceOf(demetorZap.address))).to.lessThan(1);
...
async function getOdosRequest(request) {
    ...
    try {
        ...
        console.log("[demetorZap] getSwapTransaction: " + e);
        return 0;
    }

    if (transaction.statusCode === 400) {
        console.log(`[demetorZap]  ${transaction.description}`);
        return 0;
    }

    if (transaction.data.transaction === undefined) {
        console.log("[demetorZap] transaction.tx is undefined");
        return 0;
    }
