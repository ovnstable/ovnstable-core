...
describe("RebalancerPlus", function () {
    ...
    describe("approveAmountRequirement", function () {

        sharedBeforeEach("approveAmountRequirement", async () => {
        ...
        it("[1kk, 1kk]", async function () {

            await mockBalances(1000000, 1000000)
        ...
        it("[100k, 1kk]", async function () {

            await mockBalances(100000, 1000000)
        ...
        it("[400k, 1kk]", async function () {

            await mockBalances(400000, 1000000)
            ...
            let amount = await rebalancerPlus.callStatic.approveAmountRequirement(mockLinearPool.address, 100000);
            expect(amount[0]).to.equal(350000);
        });


        async function mockBalances(amount0, amount1) {
            ...
            await mockVault.mock.getPoolTokens.returns(
                ...
                [
                    amount0, amount1 // current balances [100k, 1kk]
                ],
                0 // lastChangeBlock
            );
        }
