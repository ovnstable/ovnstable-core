...
async function main() {
    ...
    let amountFrom = await usdb.balanceOf(thrusterSwap.address);
    ...
    let amountTo = await usdb.balanceOf(thrusterSwap.address);
    console.log("DEBUG: amountTo =", amountTo.toString());

    console.log((await thrusterSwap.netAssetValue()).toString())
}
