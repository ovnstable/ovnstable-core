...
contract ControlOpUsdc is Initializable, AccessControlUpgradeable, UUPSUpgradeable {

    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");
    bytes32 public constant STRATEGY_ROLE = keccak256("STRATEGY_ROLE");

    StrategyOpUsdc public strategy;
    ...
    function setStrategy(address payable _strategy) external onlyStrategy{
        strategy = StrategyOpUsdc(_strategy);
    ...
    function aavePool() public view returns (ILendingPool) {
        return ILendingPool(aavePoolAddressesProvider.getLendingPool());
    }
    ...
    */
    ...
    function currentLiquidity() public view returns (Liquidity memory) {
        ...
        uint256 poolTokenUsd = opToUsd(poolToken);
        ...
        uint256 opBalanceUsd = opToUsd(op.balanceOf(address(strategy)));
        ...
        console.log("opBalanceUsd  ", opBalanceUsd);
        console.log("-----------------");
    ...
    function currentAmounts() public view returns (Amounts memory) {
        ...
        uint256 aaveCollateralAmount = aaveCollateralUsd * usdcDm / uint256(oracleUsdc.latestAnswer());

        uint256 aaveBorrowAmount = aaveBorrowUsd * opDm / uint256(oracleOp.latestAnswer());
    ...
    function balances() external view returns (IHedgeStrategy.BalanceItem[] memory) {
        ...
        items[0] = IHedgeStrategy.BalanceItem(address(op), toUint256(liq.borrowToken), amounts.borrowToken, true);
        items[1] = IHedgeStrategy.BalanceItem(address(usdc), toUint256(liq.collateralAsset), amounts.collateralAsset, false);
        items[2] = IHedgeStrategy.BalanceItem(address(op), toUint256(liq.poolToken), amounts.poolToken, false);
        ...
        items[6] = IHedgeStrategy.BalanceItem(address(op), toUint256(liq.freeToken), amounts.freeToken, false);
        return items;
    }
     ...
     */
    function _getLiquidity() internal view returns (uint256 opBalance, uint256 usdcBalance) {
        uint256 lpTokenBalance = gauge.balanceOf(address(strategy));
        uint256 totalLpBalance = pair.totalSupply();
        (uint256 reserveOp, uint256 reserveUsdc,) = pair.getReserves();
        opBalance = reserveOp * lpTokenBalance / totalLpBalance;
        usdcBalance = reserveUsdc * lpTokenBalance / totalLpBalance; 
    }

    // Current price op/usdc in mesh pool in USD/USD in e+2
    function _pricePool() internal view returns (uint256){
        ...
        // token 1 - usdc
        (uint256 reserveOp, uint256 reserveUsdc,) = pair.getReserves();
        uint256 reserveOpUsd = opToUsd(reserveOp);
        uint256 reserveUsdcUsd = usdcToUsd(reserveUsdc);

        console.log("----------------- priceInDystUsdpMaticPool()");
        console.log("reserveOp       ", reserveOp);
        console.log("reserveOpUsd    ", reserveOpUsd);
        ...
        return reserveUsdcUsd * 10 ** 8 / reserveOpUsd;
    }
     ...
     */
    function opToUsd(uint256 amount) public view returns (uint256) {
        // X / 100 because converting return e6+2 as oracle price but need to remove additional +2
        return amount * uint256(oracleOp.latestAnswer()) / opDm / 100;
    }
     ...
     */
    function usdToOp(uint256 liquidity) public view returns (uint256) {
        // liquidity * 100 => because need e6+2 for converting but liq in e6
        return liquidity * opDm * 100 / uint256(oracleOp.latestAnswer());
    }
    ...
    function usdToUsdc(uint256 liquidity) public view returns (uint256) {
        // liquidity * 100 => because need e6+2 for converting but liq in e6
        return liquidity * usdcDm * 100 / uint256(oracleUsdc.latestAnswer());
    }
