...
contract ProportionFacet is IProportionFacet, IChainFacet, IMathFacet {
    function getProportion(
        ...
        (uint256 decimals0, uint256 decimals1) = getPoolDecimals(pair);
        uint256 dec0 = 10 ** decimals0;
        uint256 dec1 = 10 ** decimals1;
        uint160 sqrtRatioX96 = getPoolSqrtRatioX96(pair);

        uint160 sqrtRatio0 = getSqrtRatioAtTick(tickRange[0]);
        uint160 sqrtRatio1 = getSqrtRatioAtTick(tickRange[1]);
        uint128 liquidity = getLiquidityForAmounts(sqrtRatioX96, sqrtRatio0, sqrtRatio1, dec0 * 1000, dec1 * 1000);
        (token0Amount, token1Amount) = getAmountsForLiquidity(sqrtRatioX96, sqrtRatio0, sqrtRatio1, liquidity);
    ...
    function getProportionForZap(
        ...
        (outTokens[0].token, outTokens[1].token) = getPoolTokens(pair);
        ...
        for (uint256 i = 0; i < inputTokens.length; i++) {
            ...
            uint256 amountUsd = mulDiv(inputTokens[i].price, inputTokens[i].amount, 10 ** decimals[i]);
        ...
        result.poolProportionsUsd[0] = mulDiv(sumInputsUsd, outTokens[0].prop, outTokens[1].prop);
        ...
        for (uint256 i = 0; i < 2; i++) {
            if (outTokens[i].idx < inputTokens.length && result.poolProportionsUsd[i] < outTokens[i].amountUsd) {
                outTokens[i].amountToSwap = mulDiv(outTokens[i].amountUsd - result.poolProportionsUsd[i], 10 ** decimals[outTokens[i].idx], inputTokens[outTokens[i].idx].price);
        ...
        result.outputTokenProportions[0] = mulDiv(result.poolProportionsUsd[0] - outTokens[0].amountUsd, BASE_DIV,
