...
function _addABI(abiArray) {
    ...
    if (Array.isArray(abiArray)) {
        ...
        abiArray.map(function(abi) {
            if (abi.name) {
                const signature = state.utils.sha3(
                    abi.name +
                    "(" +
                    abi.inputs
                        .map(_typeToString)
...
function _decodeMethod(data) {
    ...
    if (abiItem) {
        let decoded = state.abiCoder.decodeParameters(abiItem.inputs, data.slice(10));
        ...
        for (let i = 0; i < decoded.__length__; i++) {
            ...
            if (isUint || isInt) {
                ...
                if (isArray) {
                    parsedParam = param.map(val => new state.utils.BN(val).toString());
                } else {
                    parsedParam = new state.utils.BN(param).toString();
                }
            }
...
function _decodeLogs(logs) {
    return logs.filter(log => log.topics.length > 0).map((logItem) => {
        ...
        if (method) {
            ...
            const decodedData = state.abiCoder.decodeParameters(
                dataTypes,
                logData.slice(2)
            );
            ...
            method.inputs.map(function(param) {
                ...
                if (
                    ...
                    if (typeof decodedP.value === "string" && decodedP.value.startsWith("0x")) {
                        decodedP.value = new state.utils.BN(decodedP.value.slice(2), 16).toString(10);
                    } else {
                        decodedP.value = new state.utils.BN(decodedP.value).toString(10);
                    }
                }
...
function setAbiDecoder(coder){
    state.abiCoder = coder;
}
